  ...
  return (
    ...
    >
      <div className="flex-shrink-0 mb-4 text-center">
        <h2 className="text-2xl font-bold text-[#005B9E]">Cropping & Framing</h2>
      </div>
        ...
        <div className="flex-1 max-w-md space-y-4">
          <div className="bg-white p-4 rounded-xl shadow-sm border border-[#00A5E5]/30">
            <div className="flex items-center gap-3 mb-2">
              ...
              <p className="font-semibold text-[#005B9E]">Proper Framing</p>
            </div>
          ...
          <div
            ...
            className="flex items-center gap-3 mt-4 cursor-pointer select-none bg-white p-3 rounded-xl border border-[#005B9E]/30 hover:shadow-md transition-all"
          >
            <div className={`w-5 h-5 border-2 rounded-md flex items-center justify-center transition-colors ${checked ? "bg-[#005B9E] border-[#005B9E]" : "border-[#005B9E] bg-white"}`}>
              {checked && <CheckCircle2 className="h-4 w-4 text-white" />}
            </div>
            <label className="text-[#005B9E] text-sm font-medium cursor-pointer">
      ...
      <div className="flex justify-center gap-4 mt-6 flex-shrink-0">
        ...
        <Button disabled={!checked} onClick={onNext} className={`px-8 py-2.5 rounded-full shadow-md ${checked ? "bg-[#005B9E] hover:bg-[#00487a] text-white" : "bg-[#9cc6e4] text-white cursor-not-allowed"}`}>Continue</Button>
      </div>
    </motion.div>
  );
}
